...
function displayPagination() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    paginationButton.addEventListener("click", () => displayMenuItems(i));
...
searchButton.addEventListener('click', () => {
  ...
  const paginatedItems = menuItems.slice(0, menuItems.length);
  console.log(paginatedItems)
  paginatedItems.forEach(item => {
    if (inputValue === item.name) {

      const mealimg = document.createElement("div")
      mealimg.classList.add("col-4");
      const ItemImage = `<img src="${item.image}" alt="meal Thumbnail" class="img-thumbnail"/>`
      console.log(ItemImage)
      mealimg.innerHTML = ItemImage;
      searchContainer.appendChild(mealimg);

      const congThuc = document.createElement("div");
      congThuc.classList.add("col-8");
      const congthucmeal = `
      <p class = "adfjh"> ${item.description} </p>`
      congThuc.innerHTML = congthucmeal;
      searchContainer.appendChild(congThuc)
      return flag2 = true;
    } else {
      return flag2= false;
    }
  ...
  if(flag2){} else{
    ...
    none.classList.add("col-12");
    const None = `
    <p class = "adfjh">Không tìm thấy món ăn</p>`
    none.innerHTML = None;
    searchContainer.appendChild(none);
  }
});